...
Page({
  ...
  goSearch(e) {
    ...
    //   wx.navigateTo({
    //       url: '/pages/inforClassroom/inforClassroom?params=' + JSON.stringify(params),
    //   })
        wx.navigateBack({
            delta: 1,
        })
        wx.setStorageSync('infoClassroomSearParams', params)
    this.saveKeyWord('searHistoryInfoClassroom', inputValue, 8)
  },
  ...
  getHistory() {
    this.setData({
      searchHistory: wx.getStorageSync('searHistoryInfoClassroom')
    })
  },
  //删除历史记录
  delFun() {
    app.$u.showModal('确认清空历史记录吗').then(res => {
      this.setData({
        ...
        wx.setStorageSync('searHistoryInfoClassroom', []);
      this.getHistory();
    })
  },
