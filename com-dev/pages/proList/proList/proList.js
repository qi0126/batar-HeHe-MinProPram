...
Page({
    ...
    goSearch() {
        wx.navigateTo({
            url: '/pages/proList/proListSearch/proListSearch',
        })
    },
    ...
    // 点击更多 todo
    goMoreSeries(e) {
        const { seriesOne } = this.data
        // console.log(seriesOne)
        const { item } = e.currentTarget.dataset
        // console.log(seriesOne, item)
    ...
    getProsIndex(series) {
        ...
        if (series === '热销') {
            app.$api.produceHotSale().then(res => {
                res.data.forEach(item => {
                    if (item.list) {
                        item.list.forEach(it => {
                            it.gold = `${it.min}~${it.max}`
                            if (it.min == it.max) {
                                it.gold = it.min
                            }
                        })
        ...
        if (series === '臻品') {
            ...
            app.$api.themeSeriesSaleList(params).then(res => {
                res.data.forEach(item => {
                    if (item.list) {
                        item.list.forEach(it => {
                            it.gold = `${it.min}~${it.max}`
        ...
        if (series !== '硬金' && series !== '臻品' && series !== '热销') {
            let params = {
                one: series
            }
            app.$api.produceSIndex(params).then(res => {
                res.data.forEach(item => {
                    if (item.list) {
                        item.list.forEach(it => {
                            it.gold = `${it.min}~${it.max}`
    ...
    msgLoop(self) {
        app.$api.msgUnreadCount().then(res => {
            self.setData({
                msgStatus: res.data
            })
        })
    ...
    getData(num) {
        ...
        app.$api.produceIndex(params).then(res => {
