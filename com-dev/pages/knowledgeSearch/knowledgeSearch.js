...
Page({
    ...
    goSearch(e) {
        ...
        wx.setStorageSync('knowledgeSearParams', params)
        this.saveKeyWord('searHistoryKnowledgeClassroom', inputValue, 8)
    },
    ...
    getHistory() {
        this.setData({
            searchHistory: wx.getStorageSync('searHistoryKnowledgeClassroom')
        })
    },
    //删除历史记录
    delFun() {
        app.$u.showModal('确认清空历史记录吗').then(res => {
            ...
            wx.setStorageSync('searHistoryKnowledgeClassroom', []);
            this.getHistory();
        })
    },
